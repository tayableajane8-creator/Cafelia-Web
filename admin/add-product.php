...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($name === "" || $price === "" || $stock === "" || $category === "") {
    ...
    } elseif (!in_array($category, ["Coffee", "Beverages", "Treats", "Food"], true)) {
        ...
        if (!in_array($extension, $allowed_extensions, true)) {
            ...
            if (!is_dir($upload_dir) && !mkdir($upload_dir, 0755, true)) {
                ...
                try {
                    $image_name = "product_" . bin2hex(random_bytes(8)) . "." . $extension;
                } catch (Throwable $e) {
                ...
                if (!move_uploaded_file($file["tmp_name"], $upload_path)) {
                    $error = "The product image could not be saved.";
                } else {
                    ...
                    if (!$stmt) {
                        @unlink($upload_path);
                        $error = "Failed to prepare product creation: " . $conn->error;
                    } else {
...
body {
    ...
    color: var(--text);
    background:
        radial-gradient(circle at 82% 8%, rgba(216,163,109,.14), transparent 27%),
        linear-gradient(135deg, #f8f3eb 0%, #eee2d4 100%);
}
...
.sidebar-brand { padding: 10px 14px 30px; }
.sidebar-brand h1 {
...
.back-link {
    ...
    font-weight: 800;
    text-decoration: none;
    box-shadow: 0 8px 20px rgba(49,30,20,.04);
    transition: .2s ease;
}
.back-link:hover {
    background: #fff;
...
.form-group textarea:hover {
    border-color: rgba(185,130,82,.34);
}
...
.form-group:nth-child(7) { grid-column: 1 / -1; }
...
.file-upload input[type="file"] {
    ...
    min-height: 52px;
    padding: 8px;
    border: 1px dashed rgba(90,56,39,.28);
    border-radius: 11px;
    background: #faf6f0;
    color: var(--muted);
...
.file-upload input[type="file"]:hover {
    border-color: rgba(185,130,82,.55);
    background: #f8f1e8;
}
...
#previewImage {
    ...
    box-shadow: 0 10px 22px rgba(36,21,15,.10);
}
...
@media (max-width: 950px) {
    ...
    .admin-profile { margin-bottom: 18px; }
    .sidebar-nav { display: grid; grid-template-columns: repeat(2, 1fr); }
    .sidebar-bottom { margin-top: 12px; }
    .main { margin-left: 0; padding: 30px 22px 45px; }
}

@media (max-width: 720px) {
    .topbar { align-items: flex-start; flex-direction: column; }
    .topbar h2 { font-size: 33px; }
    .back-link { width: 100%; }
    .admin-form { grid-template-columns: 1fr; padding: 26px 22px 30px; }
    .form-group:nth-child(1),
    .form-group:nth-child(2),
...
@media (max-width: 520px) {
    ...
    .card-heading { padding: 23px 20px 20px; }
    .admin-form { padding: 23px 20px 26px; }
...
<main class="main">
    <header class="topbar">
        <div>
            <div class="topbar-kicker">Cafelia Management</div>
            <h2>Add Product</h2>
            <p>Create a new menu item and save it directly to the products database.</p>
        </div>
    ...
    <section class="add-card">
        ...
        <form method="POST" enctype="multipart/form-data" class="admin-form">
            ...
            <div class="form-group">
                ...
                <input type="number" id="stock" name="stock" min="0" step="1" value="<?php echo htmlspecialchars($stock); ?>" required>
                <small>Set the quantity currently available for this product.</small>
            </div>

            <div class="form-group">
                <label for="category">Category *</label>
                <select id="category" name="category" required>
                    <?php foreach (["Coffee", "Beverages", "Treats", "Food"] as $option): ?>
            ...
            <div class="form-group file-upload">
                ...
                <small>JPG, JPEG, PNG, or WEBP. Maximum 5MB.</small>
